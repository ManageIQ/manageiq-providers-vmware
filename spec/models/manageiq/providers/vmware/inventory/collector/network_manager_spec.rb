describe ManageIQ::Providers::Vmware::Inventory::Collector::NetworkManager do
  describe "Utility" do
    before do
    ...
    subject { described_class.new(ems, network) }
    ...
    describe 'build_vapp_network' do
      [
        ...
        it "build_vapp_network - #{test_case[:name]}" do
          ...
          n = subject.send(:build_vapp_network, vapp, 'network-id', test_case[:net_conf])
    ...
    describe 'network_id_from_links' do
      [
        ...
        it "network_id_from_links - #{test_case[:name]}" do
          expect(subject.send(:network_id_from_links, test_case[:data])).to eq(test_case[:expected])
        end
      end
    end
    ...
    describe 'parent_vdc_network' do
      [
        ...
        it "parent_vdc_network - #{test_case[:name]}" do
          expect(subject.send(:parent_vdc_network, test_case[:net_conf], test_case[:vdc_networks]))
    ...
    describe 'corresponding_vdc_network' do
      [
        ...
        it "parent_vdc_network - #{test_case[:name]}" do
          expect(subject.send(:corresponding_vdc_network, test_case[:net_conf], test_case[:vdc_networks]))
            .to eq(test_case[:expected])
        end
      end
    end
    ...
    describe 'fetch_network_configurations_for_vapp' do
      [
        ...
        it "to_list - #{test_case[:name]}" do
          ...
          expect(subject.send(:fetch_network_configurations_for_vapp, 'vapp-id')).to eq(test_case[:expected])
        end
      end
    end
    ...
    describe 'fetch_nic_configurations_for_vm' do
      [
        ...
        it "to_list - #{test_case[:name]}" do
          ...
          expect(subject.send(:fetch_nic_configurations_for_vm, 'vm-id')).to eq(test_case[:expected])
        end
      end
    end
    ...
    def mock_api_response(fun_name, response)
      ...
      subject.instance_variable_set(:@connection, d)
    end
  end
end
