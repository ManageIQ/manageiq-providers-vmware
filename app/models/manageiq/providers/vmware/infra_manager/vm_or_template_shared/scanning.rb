    ...
    if ost.config && ost.config.capabilities[:vixDisk] == true
      ...
      if miqVimHost
        ...
        begin
          ...
          ost.miqVim = MiqFaultTolerantVim.new(:ip => miqVimHost[:address], :user => miqVimHost[:username], :pass => password_decrypt, :use_broker => use_broker, :vim_broker_drb_uri => ost.scanData['ems'][:vim_broker_drb_uri])
